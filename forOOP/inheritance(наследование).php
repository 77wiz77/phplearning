...
Чтобы наследовать один класс от другого, нам надо применить инструкцию extends. Например, унаследуем класс Customer от класса User:*/
  ...
  class Customer extends User //класс Customer наследуется от User
  {
    ...
    function __construct($name, $age, $acc)
    {
        ...
        $this->account = $acc; //номер счета / приравнием переменную $account к значению подставленному в конструктор (acc)
        //Например, чтобы получить значение свойства $name, надо использовать выражение $this->name. 
        $this->sum = 20; //сумма будет равна: 20 руб
    }
  ...
  И несмотря на то, что свойство $name определено в классе User, его также можно получить из класса Customer: $this ->name
  Если в классе-наследнике определен метод с тем же именем, что и в классе-родителе, 
  то метод класса-наследника замещает метод класса-родителя. 
  В нашем случае в обоих классах определен метод getClassInfo(), поэтому Customer будет обращаться к своему методу getClassInfo().
  
  Если нам надо обратиться к методу родительского класса, то мы можем использовать инструкцию parent:*/
  //parent::getInfo();

  /*И при вызове метода getInfo() из класса Customer сначала будет вызываться метод getInfo() родительского класса User, 
  а затем остальные инструкции:*/
  // function getInfo()
  // {
  //     parent::getInfo();
  //     echo "Номер счета $this->account ; Сумма: $this->sum <br>";
...
<?php //Запрет наследования и инструкция final
  /*В примере выше методы getInfo() и getClassInfo() переопределяются классом-наследником. 
  Однако иногда возникают ситуации, когда надо запретить переопределение методов. 
  Для этого в классе-родителе надо указать методы с модификатором final:*/
  class User1
  {
      //.........................
  ...
  //В этом случае во всех классах-наследниках от класса User1 мы уже не сможем определить метод с таким же именем.
?>